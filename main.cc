...
Vec3f cast_ray(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, const std::vector<Light> &lights, size_t depth=0) {
    ...
    if (depth>4 || !scene_intersect(orig, dir, spheres, point, N, material)) {
        return Vec3f(0.2, 0.2, 0.6); // background color
    }
...
void ppmout(const std::string fname, std::vector<Vec3f> framebuffer, const int width, const int height) {
...
void render(int width,
  ...
  for (size_t j = 0; j < height; j++) {
    for (size_t i = 0; i < width; i++) {
      framebuffer[i + j * width] = Vec3f(i / float(width), j / float(height), i / float(width));
    }
  }
...
int main(int argc, const char **argv) {
  ...
  std::cout << "Rendering " << frames << " frames\n";
	for (int i = 0; i < frames; i++) {
		...
		spheres.push_back(Sphere(Vec3f( 7 - (i * (15. / frames)),    5 - (i *  (14. / frames)),   -18), 4,     mirror));
